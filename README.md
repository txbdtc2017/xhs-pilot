...
如果你的 Kimi coding 套餐提供 Anthropic-compatible endpoint，推荐把文本分析、策略和正文生成切到 `anthropic-messages`，而把图片分析与 embedding 继续交给独立 provider：
...
- 当前项目不会把 `embedding` 自动切到 `anthropic-messages`，所以录入和检索链路仍然需要有效的 `EMBEDDING_*`
