...
- 使用 `generateObject()` + `analysisSchema` 约束输出
- 使用 `generateObject()` + `visualAnalysisSchema` 做视觉分析（含 OCR）
- `temperature=0`
...
- ❌ 不要实现创作生成功能
- ❌ 不要实现风格画像的自动归纳
- ❌ 不要生成多种类型的 embedding（Phase 1 只生成一种：title+body 拼接）
...
- [ ] 样本状态从 `draft` → `analyzed` → `reviewed` 正确流转
